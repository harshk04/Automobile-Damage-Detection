...
def generate_pdf(car_details, prediction_json, total_price, price_details, image_path):
    ...
    elements.append(Paragraph("Car Damage Detection Results", title_style))
    ...
    car_info_data = [
        ['Car Number', car_details['Registration']],
        ['Brand', car_details['Car Brand']],
        ['Color', car_details['Colour']],
        ['Model', car_details['Model']],
        ['Type', car_details['Type']],
        ['Fuel Type', car_details['Fuel']],
        ['Year of Manufacture', car_details['Year of Manufacture']],
        ['Car Resell Value', car_details['Car Price']]
    ]
    ...
    damage_data = [['Confidence', 'Class', 'Price']]
    for detail in price_details:
        damage_data.append([f"{detail[0]:.2f}", detail[1], f"{detail[2]:.2f}"])
    ...
    # Add total price
    elements.append(Spacer(1, 20))
    total_price_paragraph = Paragraph(f"Total estimated price: {total_price:.2f}", styles['Heading2'])
